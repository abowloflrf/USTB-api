...
class AuthController extends Controller
{
    ...
    public function getInnovativeCredit()
    {
        ...
        foreach ($tableBody->childNodes as $table)
            ...
            array_push($returnData, $singleData);
        }
        $returnData['total_credit'] = $totalCredit;
    ...
    public function getScore()
    {
        ...
        $returnData = [];
        //初始化curl
        $ch = curl_init("http://elearning.ustb.edu.cn/choose_courses/information/singleStuInfo_singleStuInfo_loadSingleStuScorePage.action");
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Cookie: JSESSIONID=' . $this->JSEESIONID));
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_HEADER, 0);
        $output = curl_exec($ch);
